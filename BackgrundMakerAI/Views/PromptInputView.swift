...
struct PromptInputView: View {
    ...
    var body: some View {
        VStack(alignment: .center) {
            Text("Background Image Configuration").padding()
            Form {
                Section("Context") {
                    TextField("Describe the Image", text: $viewModel.context)
    ...
    var body: some View {
        Text("Hello")
            .sheet(isPresented: $isPresented) {
                PromptInputView(viewModel: PromptInputViewModel(prompt: $prompt))
